...
let left=cfg.seconds,running=false,interval=null,expired=false,unlocked=false,voice=null;
...
#shotPause{margin-top:6px;width:100%}
#voiceTest{margin-top:6px;width:100%}
#shotTimer.expired{border-color:#ff7474}
#voiceStatus{font-size:10px;color:#8f9b95;margin-top:5px}
...
function chooseVoice(){
 if(!('speechSynthesis' in window)){vs.textContent='音声：このブラウザでは利用できません';return}
 const vsx=speechSynthesis.getVoices();
 const ja=vsx.filter(v=>(v.lang||'').toLowerCase().startsWith('ja'));
...
function speak(text){
 ...
 try{
  ...
  u.lang='ja-JP';u.rate=0.9;u.pitch=1.15;u.volume=1;
...
function unlockVoice(){
 ...
 vs.textContent=voice?'音声：ON（'+voice.name+'）':'音声：ON';
}
if('speechSynthesis' in window){
 chooseVoice();
...
function render(){
 ...
 pb.textContent=running?'⏸ 一時停止':'▶ 開始';
...
function reset(){
 ...
 stop();left=cfg.seconds;expired=false;running=true;render();
 interval=setInterval(tick,1000);
}
function tick(){
 if(!running)return;
 left--;
 if(left<=10&&left>=1)speak(String(left));
 if(left<=0){
   left=0;stop();expired=true;
   try{navigator.vibrate&&navigator.vibrate([180,100,180,100,450])}catch(e){}
   speak('時間です！');render();return;
 }
 render();
}
document.querySelectorAll('#shotTimerBtns button').forEach(b=>b.onclick=()=>{
 unlockVoice();cfg.seconds=+b.dataset.s;save();reset();
});
pb.onclick=()=>{
 unlockVoice();
 if(expired){reset();return}
 if(running)stop();else{running=true;interval=setInterval(tick,1000)}
 render();
};
vt.onclick=()=>{unlockVoice();speak('音声テストです。聞こえていますか？')};

const op=window.pocket;if(typeof op==='function')window.pocket=function(n){unlockVoice();op(n);reset()};
const oc=window.changeTurn;if(typeof oc==='function')window.changeTurn=function(a){unlockVoice();oc(a);reset()};
const or=window.rackEnd;if(typeof or==='function')window.rackEnd=function(){unlockVoice();or();reset()};

document.addEventListener('visibilitychange',()=>{if(document.hidden&&running){/* keep state; iOS may suspend JS */}});
render();
})();