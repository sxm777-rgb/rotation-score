...
const card=document.createElement('div');card.id='shotTimer';card.innerHTML='<div class="tt">SHOT CLOCK</div><div id="shotTime">45</div><div id="shotTimerBtns"><button data-s="30">30秒</button><button data-s="35">35秒</button><button data-s="45">45秒</button><button data-s="60">60秒</button></div><button id="shotPause">一時停止</button>';
...
function render(){te.textContent=left;te.className=left<=5?'danger':left<=10?'warning':'';card.classList.toggle('expired',expired);pb.textContent=running?'一時停止':'再開';document.querySelectorAll('#shotTimerBtns button').forEach(b=>b.classList.toggle('active',+b.dataset.s===cfg.seconds))}
function reset(){clearInterval(interval);left=cfg.seconds;running=true;expired=false;render();interval=setInterval(()=>{if(!running)return;left--;if(left<=0){left=0;clearInterval(interval);interval=null;running=false;expired=true;try{navigator.vibrate&&navigator.vibrate([180,100,180,100,300])}catch(e){}}render()},1000)}
document.querySelectorAll('#shotTimerBtns button').forEach(b=>b.onclick=()=>{cfg.seconds=+b.dataset.s;save();reset()});
pb.onclick=()=>{if(expired){reset();return}running=!running;if(running&&!interval)interval=setInterval(()=>{},1000);render()};
